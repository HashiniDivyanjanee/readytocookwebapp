...
const MenuItemCard = ({ item, index, addToCart }) => {
  ...
  const weightOptions = isReadymade ? ["Full (1Kg)", "Half (500g)"] : ["500g", "1kg"];
  ...
  const CustomSelect = ({ label, value, options, onChange, type }) => (
    <div className="relative space-y-1.5" onClick={(e) => {
      ...
      <label className="text-[8px] font-black uppercase tracking-widest text-gray-400 block">{label}</label>
      <div className={`w-full bg-gray-50 border ${activeDropdown === type ? "border-[#FF5C00]" : "border-gray-100"} rounded-xl px-2 py-2 text-[9px] font-black uppercase text-gray-900 flex justify-between items-center transition-all h-9`}>
        <span className="truncate">{value}</span>
        <svg className={`w-2 h-2 shrink-0 ml-1 transition-transform ${activeDropdown === type ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="group bg-white rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-[0_30px_60px_rgba(0,0,0,0.1)] transition-all duration-500 flex flex-col border border-gray-100/50 cursor-pointer"
      ...
      <div className="p-8 flex flex-col flex-1 space-y-4">
        ...
        <p className="text-gray-500 text-sm font-medium leading-relaxed flex-1 line-clamp-2">{item.desc}</p>
        
        {/* Customization Grid */}
        <div className="grid grid-cols-3 gap-2 pt-2 border-t border-gray-50">
          <CustomSelect label={weightLabel} value={weight} options={weightOptions} onChange={setWeight} type="weight" />
        ...
        {filteredItems.length > 0 ? (
          <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 transition-all duration-700 ${animate ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
            {filteredItems.map((item, index) => (
              <MenuItemCard key={item.id} item={item} index={index} addToCart={addToCart} />
            ))}
          </div>
        ) : (
